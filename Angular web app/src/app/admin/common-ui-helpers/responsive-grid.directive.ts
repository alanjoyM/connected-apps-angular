...
@Directive({
  selector: 'responsive-grid[responsive]',
  exportAs :'ResponsiveGridDirective'
})
export class ResponsiveGridDirective implements OnInit, OnChanges, OnDestroy{
  ...
  @Injectable() private matGridList: MatGridList
  ...
  ngOnDestroy() {
    if (this.breakPointObserverSubscription) {
      ...
      this.breakPointObserverSubscription = new Subscription();
    }
  }
